...
def generate_chunks(input_img_directory, output_directory, n_channels=1, sample_id=None, model_file=None, chunk_overlap=[16, 16, 8],
                    pred_threshold=0.5, int_threshold=200, normalize_intensity=True, resample_chunks=False, use_mask=False,
                    mask_file=None, acquired_img_resolution=[0.75, 0.75, 4], tree_radius=2, measure_coloc=False, resample_resolution=25, 
                    trained_img_resolution=[0.75, 0.75, 2.5], chunk_size=[112, 112, 32]
    ...
    save_name = os.path.join(output_directory, sample_id + ".csv")
    ...
    chunk_start = np.arange(0, total_slices, step=(chunk_size[2] - chunk_overlap[2]))
    ...
    for n in range(n_chunks):
        ...
        x_positions = np.arange(0, cols, step=(chunk_size[0] - chunk_overlap[0]))
        y_positions = np.arange(0, rows, step=(chunk_size[1] - chunk_overlap[1]))
            ...
            for j in range(n_chunks_c):
                if empty_idx[a] != 1:
                    ...
                    if centroids.any():
                        ...
                        centroids = centroids[centroids[:, 0] > chunk_overlap[0] / 2]
                        centroids = centroids[centroids[:, 0] <= chunk_size[0] - chunk_overlap[0] / 2]

                        centroids = centroids[centroids[:, 1] > chunk_overlap[1] / 2]
                        centroids = centroids[centroids[:, 1] <= chunk_size[1] - chunk_overlap[1] / 2]

                        centroids = centroids[centroids[:, 2] > chunk_overlap[2] / 2]
                        centroids = centroids[centroids[:, 2] <= chunk_size[2] - chunk_overlap[2] / 2]
...
# Call generate_chunks() when this script is run directly
